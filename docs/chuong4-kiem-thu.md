...
| 14 | Policy System | Policy Library, Policy Assignments (ALLOW/DENY), effective permissions hợp nhất |
...
Tổng cộng **9 tệp spec** kiểm thử trực tiếp REST API tại `tests/api/`, thực hiện các yêu cầu HTTP thực sự đến server chạy trên port 4010.
...
#### TC-API-08: Policy System (`policy-system.spec.ts`)

| ID | Mô tả | Input | Kết quả mong đợi |
|----|-------|-------|-----------------|
| TC-API-08-1 | Lấy danh sách policies | `GET /api/v1/admin/permissions/policies` (admin) | HTTP 200; `data` là mảng, ít nhất 1 policy tồn tại sau migration |
| TC-API-08-2 | Lấy permission catalog | `GET .../policies/permission-catalog` | HTTP 200; mảng có đủ trường `id`, `name`, `resource`, `action` |
| TC-API-08-3 | Lấy danh sách principals | `GET .../policies/principals` | HTTP 200; `data.users`, `data.groups`, `data.ous` đều là mảng |
| TC-API-08-4 | CRUD vòng đời đầy đủ: tạo → đặt quyền → thêm assignment → xóa assignment → xóa policy | Nhiều request tuần tự | Mỗi bước trả đúng HTTP status; cleanup xóa policy thành công |
| TC-API-08-5 | DENY assignment phản ánh đúng trong effective permissions | Tạo policy DENY, gán cho admin user, kiểm tra `/effective/system-users/:id` | Permission bị DENY xuất hiện trong `denied[]`, không có trong `allowed[]` |
| TC-API-08-6 | Endpoint yêu cầu xác thực | `GET /policies` không có token | HTTP 401 |
| TC-API-08-7 | System policy không thể xóa | `DELETE` policy có `isSystem=true` | HTTP 403 |
| TC-API-08-8 | Effective perms response có đủ sources | `GET /effective/system-users/:id` | `sources.policyAllowed` và `sources.policyDenied` đều là mảng |

#### TC-API-09: Permission Center (`permission-center.spec.ts`)

| ID | Mô tả | Input | Kết quả mong đợi |
|----|-------|-------|-----------------|
| TC-API-09-1 | Lấy unified roles và permissions | `GET /api/v1/admin/permissions/roles`, `GET .../permissions` | HTTP 200; `data.classic` và `data.directory` đều là mảng |
| TC-API-09-2 | Classic role CRUD qua namespace thống nhất | `POST/PUT/DELETE /classic/roles`, `PUT .../permissions` | Tạo 201; cập nhật 200; xóa 200; quyền được gán đúng |
| TC-API-09-3 | Lấy effective permissions qua unified endpoint | `GET /effective/system-users/:id` (me) | HTTP 200; đủ 5 nguồn: `classic`, `directoryAllowed`, `directoryDenied`, `policyAllowed`, `policyDenied` |
| TC-API-09-4 | Endpoint yêu cầu xác thực | `GET /permissions/roles` không có token | HTTP 401 |
...
Toàn bộ test suite được thực thi trên môi trường local (Windows 11, Node.js 20.x, PostgreSQL 16) ngày **22/03/2026** với dữ liệu seed đầy đủ. Kết quả được ghi nhận từ **Playwright Test Runner** sau khi chạy lệnh `pnpm test:e2e`.

#### Tổng quan số lượng ca kiểm thử (kết quả thực tế)

| Loại kiểm thử | Số tệp spec | Tổng ca | Đạt (Passed) | Thất bại | Bỏ qua | Tỷ lệ đạt |
|--------------|-------------|---------|-------------|----------|--------|----------|
| API Tests | 9 | 27 | **27** | 0 | 0 | **100%** |
| UI E2E Tests | 30 | 392 | **374** | 6 | 17 | **95.4%** |
| **Tổng cộng** | **39** | **419** | **401** | **6** | **17** | **95.7%** |

> _Thời gian chạy: API tests — 24.3 giây; UI E2E tests — 13 phút 15 giây_
...
| Kho hàng (UI — CRUD kho) | 2 | 2 | 0 | 100% |
...
| Policy System (API) | 8 | 8 | 0 | 100% |
| Permission Center (API) | 4 | 4 | 0 | 100% |

#### Biểu đồ kết quả tổng quan

```
API Tests (27 ca):       ████████████████████ 100% PASS
UI E2E Tests (392 ca):  ████████████████████░ 95.4% PASS
...
| TC-API-08-1 | Danh sách policies | ✅ PASS | Có policy sau khi migrate từ roles |
| TC-API-08-2 | Permission catalog | ✅ PASS | Mảng permissions đủ trường |
| TC-API-08-3 | Danh sách principals | ✅ PASS | users/groups/ous là mảng |
| TC-API-08-4 | CRUD vòng đời policy | ✅ PASS | Tạo → đặt quyền → assignment → xóa |
| TC-API-08-5 | DENY phản ánh effective perms | ✅ PASS | Permission trong `denied[]`, không có trong `allowed[]` |
| TC-API-08-6 | Auth guard policy endpoints | ✅ PASS | HTTP 401 khi không có token |
| TC-API-08-7 | System policy bất khả xâm phạm | ✅ PASS | HTTP 403 khi xóa system policy |
| TC-API-08-8 | Sources đầy đủ trong effective perms | ✅ PASS | `policyAllowed` và `policyDenied` hiện diện |
| TC-API-09-1 | Unified roles và permissions | ✅ PASS | classic + directory arrays |
| TC-API-09-2 | Classic role CRUD | ✅ PASS | Tạo/cập nhật/xóa/đặt quyền đúng |
| TC-API-09-3 | Effective perms unified endpoint | ✅ PASS | 5 nguồn đủ trong `sources` |
| TC-API-09-4 | Auth guard permission center | ✅ PASS | HTTP 401 khi không có token |
...
| TF-05 | CMDB Vietnamese tab labels | ❌ FAIL | Assertion `toContain('Loại CI')` nhưng trang load chưa hoàn tất (timing) |
| TF-06 | Requests Vietnamese labels | ❌ FAIL | Assertion `toMatch(/Yêu cầu/)` nhưng script SvelteKit chưa inject (timing) |

> **Nhận xét:** Tất cả 6 ca thất bại còn lại là do vấn đề test assertion timing, không phải lỗi chức năng. Warehouse CRUD (TF-07~11 cũ) đã được viết lại với `data-testid` đúng và hiện qua 100%. Cải thiện so với lần chạy 12/03/2026: tỷ lệ đạt tăng từ 93.2% → 95.7%, bổ sung 12 ca API tests mới (policy-system + permission-center), tổng thất bại giảm từ 11 → 6.
...
#### Vấn đề 5: PermissionCenterService tính effective permissions sai

**Mô tả:** Panel "Quyền thực tế" hiển thị 62 ALLOWED permissions trong khi Policy Library của admin chỉ có 54 — con số thực tế runtime enforce. Nguyên nhân: `PermissionCenterService` union cả `classicKeys` (Policy Library) lẫn `policyAllowed` (ALLOW assignments) vào `allowedSet`, khiến ALLOW assignments mở rộng trần quyền thay vì chỉ đóng vai trò thông tin.

**Phát hiện qua:** So sánh thủ công giữa Policy Library tab (54 perms) và Effective Perms tab (62 perms) trong Admin Console.

**Giải pháp:** Sửa `packages/application/src/rbac/PermissionCenterService.ts` — `allowedSet` chỉ khởi tạo từ `classicKeys` (Policy Library), sau đó trừ đi `policyDenied`. ALLOW assignments không tác động lên `allowedSet`, chỉ hiển thị trong `sources.policyAllowed` cho mục đích kiểm tra.

#### Vấn đề 6: Sidebar không ẩn ngay sau khi tắt quyền trong Policy Library

**Mô tả:** Sau khi admin tắt `analytics:read` trong Policy Library, sidebar vẫn hiển thị "Phân tích". Phải đợi 1 phút (TTL cache cũ) hoặc reload trang. Nguyên nhân có nhiều tầng: (a) `effectivePermsStore.invalidate()` xóa store → giá trị null → `getCapabilities()` fallback về `SYSTEM_ROLE_PERMISSIONS` với wildcard `['*']` cho admin; (b) `userId` không có trong localStorage (session cũ) khiến `fetchEffectivePerms()` return sớm; (c) TTL cache 5 phút quá dài.

**Phát hiện qua:** Kiểm thử thủ công — tắt `analytics:read` trong Policy Library, refresh sidebar, quan sát menu vẫn hiện.

**Giải pháp:**

- `effectivePermsStore.invalidate()` chuyển sang stale-while-revalidate (chỉ xóa sessionStorage, giữ in-memory store để tránh null → wildcard fallback).
- Thêm fallback decode `userId` từ JWT payload khi `localStorage('userId')` chưa có.
- Giảm TTL cache từ 5 phút xuống 1 phút.
- Đăng ký `onRefreshNeeded()` callback trong root layout để trigger re-fetch ngay sau khi invalidate.

#### Vấn đề 7: Menu không ẩn đúng ngay sau khi đăng nhập lần đầu

**Mô tả:** Sau khi đăng nhập, sidebar hiển thị menu theo quyền mặc định hardcode (`SYSTEM_ROLE_PERMISSIONS`) thay vì Policy Library. Phải reload trang một lần mới có đúng. Nguyên nhân: root layout dùng `$effect` + `effectivePermsStore.subscribe()` để cập nhật `_permsCache` (`$state`), nhưng khi `fetchEffectivePerms()` async callback gọi `effectivePermsStore.set()`, trigger vào `$state` từ bên ngoài Svelte reactivity system không đảm bảo re-render DOM ngay lập tức. Sau reload: sessionStorage cache có sẵn → `_initAllowed` non-empty → render đúng ngay.

**Phát hiện qua:** Kiểm thử thủ công — đăng nhập với tài khoản có policy tùy chỉnh, quan sát sidebar trước và sau reload.

**Giải pháp:** Thay thế `_permsCache` + `$effect(subscribe)` bằng `$allowedPerms` derived store (từ `svelte/store`) dùng trực tiếp trong `$derived.by`. Svelte 5 tự động tạo reactive subscription cho `$allowedPerms` trong `$derived.by`, đảm bảo `capabilities` recompute ngay khi `fetchEffectivePerms()` hoàn thành — không có async-to-`$state` gap.
