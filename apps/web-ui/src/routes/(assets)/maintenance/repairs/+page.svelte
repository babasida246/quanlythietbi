<script lang="ts">
  ...
  import { Plus, RefreshCw, Wrench, ArrowLeft, Info } from 'lucide-svelte';
  ...
  // ── URL-param pre-fill (from maintenance ticket) ────────────────
  const fromTicketId  = $derived(page.url.searchParams.get('fromTicket') ?? '');
  const fromAssetId   = $derived(page.url.searchParams.get('assetId') ?? '');
  const fromTitle     = $derived(decodeURIComponent(page.url.searchParams.get('title') ?? ''));
  ...
  async function loadData(p = 1) {
    try {
      ...
      const [repairResp, summaryResp, assetsResp] = await Promise.all([
        listRepairOrders({ q: q.trim() || undefined, status: status || undefined, page: p, limit: meta.limit }),
  ...
  async function submitCreate() {
    try {
      ...
      createOpen = false;
      resetCreateForm();
      // Clear URL params after create
      if (fromTicketId) {
        void goto('/maintenance/repairs', { replaceState: true });
      } else {
        await loadData(1);
      }
    } catch (err) {
  ...
  onMount(() => {
    void loadData(1);
    // If navigated from a ticket, auto-open create form and pre-fill
    if (fromTicketId) {
      createOpen = true;
      if (fromAssetId) assetId = fromAssetId;
...
<div class="page-shell page-content py-4">
  ...
  <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
    <div class="flex items-center gap-3">
      <Button variant="secondary" size="sm" onclick={() => goto('/maintenance')}>
        <ArrowLeft class="h-4 w-4 mr-1" /> Bảo trì
      </Button>
      <div>
  ...
  {#if !loading && summary}
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <StatsCard label="Tổng đơn"     value={formatNumber(summary.total)}       tone="neutral"  testid="repair-summary-total" />
      <StatsCard label="Đang xử lý"   value={formatNumber(summary.activeCount)}  tone="warning"  hint="open + diagnosing + waiting_parts" testid="repair-summary-active" />
      <StatsCard label="Đã đóng"      value={formatNumber(summary.closedCount)}  tone="success"  testid="repair-summary-closed" />
      <StatsCard label="Tổng chi phí" value={formatCurrencyVND(summary.totalCost)} tone="primary" hint="nhân công + linh kiện" testid="repair-summary-total-cost" />
    </div>
    <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
      <MiniStat label="Tổng ngừng hoạt động" value={formatDuration(summary.totalDowntimeMinutes)} tone="warning" testid="repair-summary-downtime" />
      <MiniStat label="TB thời gian xử lý"   value="{formatNumber(summary.avgResolutionHours, 1)} giờ" tone="neutral" testid="repair-summary-avg-resolution-hours" />
    </div>
    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
      <BreakdownCard title="Theo trạng thái"    items={statusBreakdown}  total={summary.total} />
      <BreakdownCard title="Theo mức độ"        items={severityBreakdown} total={summary.total} />
      <BreakdownCard title="Theo loại sửa chữa" items={typeBreakdown}    total={summary.total}
  ...
  {#if createOpen}
    <div class="card" data-testid="repairs-create-form">
      ...
      <div class="card-body space-y-4">
        <!-- Banner: navigated from a ticket -->
        {#if fromTicketId}
          <div class="flex items-start gap-2 rounded-lg border border-blue-600/40 bg-blue-900/20 px-3 py-2 text-sm text-blue-300">
            <Info class="mt-0.5 h-4 w-4 shrink-0" />
            <span>
              Tạo từ phiếu bảo trì&nbsp;
              <button
                class="font-medium underline hover:text-blue-200"
                onclick={() => goto('/maintenance')}
              >{fromTitle || fromTicketId}</button>.
              Thông tin tài sản và tiêu đề đã được điền sẵn.
            </span>
          </div>
        {/if}
              ...
              <tr>
                ...
                <td class="cell-actions">
                  <Button
                    ...
                    onclick={() => goto(`/maintenance/repairs/${repair.id}`)}
                  >
                    Chi tiết
                  </Button>
                </td>
