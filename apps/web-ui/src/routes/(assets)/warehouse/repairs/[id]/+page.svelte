<script lang="ts">
  ...
  async function handleAddPart() {
    if (!order) return;
    try {
      if (!partId && !partName.trim()) {
        throw new Error('Chon part hoac nhap ten part');
      }
      if (partId && !warehouseId) {
        throw new Error('Chon kho cho stocked part');
      }
...
<div class="space-y-5">
  ...
  <div class="flex flex-wrap items-center justify-between gap-2">
    <div class="flex items-center gap-2">
      <Button variant="secondary" size="sm" data-testid="repair-detail-back" onclick={() => goto('/warehouse/repairs')}>
        <ArrowLeft class="w-4 h-4 mr-1" /> Quay lại
      </Button>
