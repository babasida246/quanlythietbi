...
<div class="bg-surface-2 border border-slate-700 rounded-lg p-4">
  <div class="grid grid-cols-1 md:grid-cols-8 gap-4">
    <div>
      <label class="label-base mb-2">{$isLoading ? 'Search' : $_('common.search')}</label>
      <div class="relative">
        <Search class="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
        <input class="input-base pl-9" bind:value={query} placeholder={$isLoading ? 'Asset code, hostname, IP...' : $_('assets.searchPlaceholder')} />
      </div>
    </div>

    <div>
      <label class="label-base mb-2">{$isLoading ? 'Status' : $_('assets.filters.status')}</label>
      <select class="select-base" bind:value={status}>
    ...
    <div>
      <label class="label-base mb-2">{$isLoading ? 'Warranty' : $_('assets.filters.warranty')}</label>
      <select class="select-base" bind:value={warrantyExpiringDays}>
        <option value="">{$isLoading ? 'All' : $_('common.all')}</option>
    ...
    <div>
      <label class="label-base mb-2">{$isLoading ? 'Sort' : $_('assets.filters.sort')}</label>
      <select class="select-base" bind:value={sort}>
    ...
    <div>
      <label class="label-base mb-2">{$isLoading ? 'Category' : $_('assets.filters.category')}</label>
      <select class="select-base" bind:value={categoryId}>
        <option value="">{$isLoading ? 'All' : $_('common.all')}</option>
    ...
    <div>
      <label class="label-base mb-2">{$isLoading ? 'Vendor' : $_('assets.filters.vendor')}</label>
      <select class="select-base" bind:value={vendorId}>
        <option value="">{$isLoading ? 'All' : $_('common.all')}</option>
    ...
    <div>
      <label class="label-base mb-2">{$isLoading ? 'Model' : $_('assets.filters.model')}</label>
      <select class="select-base" bind:value={modelId}>
        <option value="">{$isLoading ? 'All' : $_('common.all')}</option>
    ...
    <div>
      <label class="label-base mb-2">{$isLoading ? 'Location' : $_('assets.filters.location')}</label>
      <select class="select-base" bind:value={locationId}>
        <option value="">{$isLoading ? 'All' : $_('common.all')}</option>
