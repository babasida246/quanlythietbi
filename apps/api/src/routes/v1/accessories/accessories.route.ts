...
import type { AccessoryService } from '@qltb/application';
import {
...
export async function accessoriesRoute(
    fastify: FastifyInstance,
    opts: { accessoryService: AccessoryService }
): Promise<void> {
    const { accessoryService } = opts;
    ...
    await fastify.register(async (accessoryApp) => {
        ...
        accessoryApp.get('/', {
            ...
            const result = await accessoryService.listAccessories(query);
            return result;
        });
        ...
        accessoryApp.get('/:id', {
            ...
            const accessory = await accessoryService.getAccessory(id);
            if (!accessory) {
        ...
        accessoryApp.post('/', {
            ...
            const accessory = await accessoryService.createAccessory(data, userId);
        ...
        accessoryApp.patch('/:id', {
            ...
            try {
                const accessory = await accessoryService.updateAccessory(id, data, userId);
                if (!accessory) {
                    return reply.code(404).send({ error: { code: 'NOT_FOUND', message: 'Accessory not found' } });
                }
                return accessory;
        ...
        accessoryApp.delete('/:id', {
            ...
            try {
                const deleted = await accessoryService.deleteAccessory(id, userId);
                if (!deleted) {
                    return reply.code(404).send({ error: { code: 'NOT_FOUND', message: 'Accessory not found' } });
                }
        ...
        accessoryApp.post('/:id/checkout', {
            ...
            try {
                const checkout = await accessoryService.checkoutAccessory({
                    ...data,
                    accessoryId: id
                }, userId);
        ...
        accessoryApp.get('/:id/checkouts', {
            ...
            const checkouts = await accessoryService.getCheckouts(id, activeOnly);
        ...
        accessoryApp.post('/:id/checkouts/:checkoutId/checkin', {
            ...
            try {
                const checkout = await accessoryService.checkinAccessory(
                    checkoutId,
        ...
        accessoryApp.post('/:id/adjust-stock', {
            ...
            try {
                const adjustment = await accessoryService.adjustStock({
                    ...data,
                    accessoryId: id
                }, userId);
        ...
        accessoryApp.get('/:id/adjustments', {
            ...
            const { id } = idParamSchema.parse(request.params);
            const adjustments = await accessoryService.getStockAdjustments(id);
        ...
        accessoryApp.get('/:id/audit', {
            ...
            const { id } = idParamSchema.parse(request.params);
            const logs = await accessoryService.getAuditLogs(id);
        ...
        accessoryApp.get('/alerts/out-of-stock', {
            ...
            const items = await accessoryService.getOutOfStockItems();
            return { data: items, total: items.length };
        });
        ...
        accessoryApp.get('/alerts/overdue', {
            ...
            const checkouts = await accessoryService.getOverdueCheckouts();
            return { data: checkouts, total: checkouts.length };
        });
    ...
    await fastify.register(async (categoryApp) => {
        ...
        categoryApp.get('/', {
            ...
            const categories = await accessoryService.getCategories();
        ...
        categoryApp.get('/:id', {
            ...
            const { id } = idParamSchema.parse(request.params);
            const category = await accessoryService.getCategory(id);
            if (!category) {
        ...
        categoryApp.post('/', {
            ...
            const userId = request.user?.id || 'system';
            const category = await accessoryService.createCategory(data, userId);
    ...
    await fastify.register(async (manufacturerApp) => {
        ...
        manufacturerApp.get('/', {
            ...
            const manufacturers = await accessoryService.getManufacturers();
        ...
        manufacturerApp.get('/:id', {
            ...
            const { id } = idParamSchema.parse(request.params);
            const manufacturer = await accessoryService.getManufacturer(id);
            if (!manufacturer) {
        ...
        manufacturerApp.post('/', {
            ...
            const userId = request.user?.id || 'system';
            const manufacturer = await accessoryService.createManufacturer(data, userId);
