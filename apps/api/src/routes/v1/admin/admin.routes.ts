...
export const adminRoutes: FastifyPluginAsync<AdminRoutesOptions> = async (
    ...
    fastify.get('/rbac/permissions', async (request, reply) => {
        await requirePermission(request, 'admin:roles')
        const rows = await pgClient.query<{
    ...
    fastify.get('/rbac/roles/:slug/permissions', async (request, reply) => {
        await requirePermission(request, 'admin:roles')
        const slug = (request.params as { slug: string }).slug
        const rows = await pgClient.query<{ permission_id: string; name: string; resource: string; action: string }>(
    ...
    fastify.put('/rbac/roles/:slug/permissions', async (request, reply) => {
        ...
        const roleRes = await pgClient.query<{ id: string; is_system: boolean }>(
        ...
        try {
            ...
            await pgClient.query(`DELETE FROM role_permissions WHERE role_id = $1`, [role.id])
        ...
        await appendAuditLog(pgClient, request, {
            actorUserId: ctx.userId,
            action: 'admin.rbac.role_permissions_updated',
