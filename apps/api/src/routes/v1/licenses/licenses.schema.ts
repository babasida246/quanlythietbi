...
export const licenseTypeSchema = z.enum(['per_seat', 'per_device', 'per_user', 'site_license', 'unlimited']);
export const licenseStatusSchema = z.enum(['draft', 'active', 'expired', 'retired']);
...
export const assignSeatSchema = z.object({
    ...
    (data) => {
        if (data.assignmentType === 'user') return !!data.assignedUserId;
        if (data.assignmentType === 'asset') return !!data.assignedAssetId;
        return false;
    },
    { message: 'Must provide assignedUserId for user type or assignedAssetId for asset type' }
);

export const revokeSeatSchema = z.object({ reason: z.string().optional() });

export const licenseListQuerySchema = z.object({
    page: z.coerce.number().int().min(1).default(1),
    limit: z.coerce.number().int().min(1).max(100).default(20),
    status: z.union([licenseStatusSchema, z.array(licenseStatusSchema)]).optional(),
    licenseType: licenseTypeSchema.optional(),
    supplierId: z.string().uuid().optional(),
    categoryId: z.string().uuid().optional(),
...
export const seatIdParamSchema = z.object({ id: z.string().uuid(), seatId: z.string().uuid() });
