/**
 * Audit Module - Fastify Routes (Clean Architecture)
 * Module: 07-AUDIT (Asset Audit/Inventory Check)
 */

import type { FastifyInstance, FastifyRequest, FastifyReply } from 'fastify';
import type { AuditService } from '@qltb/application';
import {
    createAuditSchema,
    updateAuditSchema,
    startAuditSchema,
...
export async function auditRoute(
    ...
    fastify.get(
        ...
        async (request: FastifyRequest, reply: FastifyReply) => {
            ...
            const result = await auditService.getMyAssignedAudits(userId);
    ...
    fastify.delete(
        ...
        async (request: FastifyRequest, reply: FastifyReply) => {
            const { id } = auditIdParamSchema.parse(request.params);
            const userId = request.user?.id || '';
            const result = await auditService.deleteAudit(id, userId);
            if (!result.success) {
                return reply.status(400).send({ error: result.error });
            }
    ...
    fastify.post(
        ...
        async (request: FastifyRequest, reply: FastifyReply) => {
            const { id } = auditIdParamSchema.parse(request.params);
            const userId = request.user?.id || '';
            const result = await auditService.submitForReview(id, userId);
            if (!result.success) {
                return reply.status(400).send({ error: result.error });
            }
            return reply.send(result.audit);
