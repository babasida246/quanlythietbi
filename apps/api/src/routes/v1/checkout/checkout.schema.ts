...
export const checkoutAssetSchema = z.object({
    ...
    (data) => {
        if (data.checkoutType === 'user' && !data.targetUserId) return false;
        if (data.checkoutType === 'location' && !data.targetLocationId) return false;
        if (data.checkoutType === 'asset' && !data.targetAssetId) return false;
        return true;
    },
    { message: 'Target must match checkout type' }
);
...
export const extendCheckoutSchema = z.object({
    ...
    newExpectedDate: z.string()
        ...
        .refine(val => new Date(val) > new Date(), { message: 'New expected date must be in the future' }),
...
export const checkoutListQuerySchema = z.object({
    ...
    status: z.union([checkoutStatusSchema, z.array(checkoutStatusSchema)]).optional(),
    checkoutType: z.union([checkoutTypeSchema, z.array(checkoutTypeSchema)]).optional(),
    dueStatus: z.union([dueStatusSchema, z.array(dueStatusSchema)]).optional(),
    assetId: z.string().uuid().optional(),
    targetUserId: z.string().uuid().optional(),
    targetLocationId: z.string().uuid().optional(),
    checkedOutBy: z.string().uuid().optional(),
    organizationId: z.string().uuid().optional(),
    dateFrom: z.string().refine(val => !val || !isNaN(Date.parse(val)), { message: 'Invalid date format' }).optional(),
    dateTo: z.string().refine(val => !val || !isNaN(Date.parse(val)), { message: 'Invalid date format' }).optional(),
...
export const checkoutIdParamSchema = z.object({ id: z.string().uuid('Invalid checkout ID') });
export const assetIdParamSchema = z.object({ assetId: z.string().uuid('Invalid asset ID') });
export const userIdParamSchema = z.object({ userId: z.string().uuid('Invalid user ID') });
