/**
 * Workflow Module – ApproverResolver
 *
 * Resolves an ApproverRule to a concrete assignee_user_id.
 * Supports: user (direct), role (first active user with that role),
 *           ou_head (department head via org_units), any_manager (fallback).
 */

import type { Pool } from 'pg';
import type { ApproverRule } from './wf.types.js';
...
export class ApproverResolver {
    ...
    async resolve(
        ...
        switch (rule.type) {
            case 'user':
                // Direct assignment: value = userId
                return rule.value ?? null;

            case 'role':
                // First active user with matching role (excluding the requester themselves)
                if (!rule.value) return null;
                return this.findFirstByRole(rule.value, requesterId);

            case 'ou_head':
                // Head of the requester's org unit
                if (!requesterOuId) return this.findAnyManager(requesterId);
                return this.findOuHead(requesterOuId, requesterId);

            case 'any_manager':
                // Any active manager / admin (fallback)
    ...
    private async findFirstByRole(role: string, excludeUserId: string): Promise<string | null> {
        ...
        const { rows: fallback } = await this.pool.query<{ id: string }>(
            `SELECT id FROM users
    ...
    private async findOuHead(ouId: string, excludeUserId: string): Promise<string | null> {
        // Look in rbac_users for the OU head (first active member of that OU)
        const { rows } = await this.pool.query<{ linked_user_id: string | null }>(
