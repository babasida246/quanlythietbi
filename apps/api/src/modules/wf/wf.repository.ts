/**
 * Workflow Request & Approval Module - Repository Layer
 * Raw SQL DB access via pg Pool
 */
...
export class WfRepository {
    ...
    /** Approvals assigned to a specific user with status=pending, including the request details.
     *  When viewAll=true (admin/manager), returns ALL pending approvals regardless of assignee. */
    async listInboxApprovals(
        assigneeId: string,
        page = 1,
        limit = 20,
        viewAll = false
    ): Promise<PaginatedResult<WfApprovalWithDetails & { request: WfRequest }>> {
        const offset = (page - 1) * limit;
        // $1=viewAll, $2=assigneeId — when viewAll=true the assignee filter is bypassed
    ...
    async updateApprovalDecision(
        ...
        // Concurrency-safe: only update if still pending (optimistic lock)
        const { rows } = await this.pool.query<Record<string, unknown>>(
            `UPDATE wf_approvals
    ...
    /** Reassign an approval to a different user (delegation) */
    ...
    /** Claim an unassigned approval (assignee_user_id IS NULL → current user) */
    ...
    /** List pending approvals that have no assignee (for admin / manager queue) */
    ...
    /** Cancel all pending approvals for a request */
    ...
    async findDefinitionByType(requestType: string): Promise<(WfDefinition & { steps: WfStep[] }) | null> {
        const defRes = await this.pool.query<Record<string, unknown>>(
        ...
        const stepsRes = await this.pool.query<Record<string, unknown>>(
    ...
    /** Map the "req_*" prefixed columns returned by inbox/unassigned queries into a WfRequestWithDetails */
