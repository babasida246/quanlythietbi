...
import type {
    ...
    StockStatus,
    AssetComponents
} from '@qltb/contracts/components';
...
function mapRowToAssignmentWithDetails(row: Record<string, unknown>): ComponentAssignmentWithDetails {
    ...
    return {
        ...
        componentType: row.component_type as ComponentAssignmentWithDetails['componentType'],
...
export class ComponentRepo {
    ...
    async findById(id: string): Promise<Component | null> {
        const result = await this.db.query('SELECT * FROM components WHERE id = $1', [id]);
    ...
    async list(query: ComponentListQuery): Promise<ComponentPaginatedResponse<ComponentWithDetails>> {
    ...
    async update(id: string, dto: UpdateComponentDto): Promise<Component | null> {
        ...
        for (const [key, column] of Object.entries(fieldMap)) {
            if (key in dto && key !== 'updatedBy') {
                fields.push(`${column} = $${paramIndex++}`);
                params.push((dto as unknown as Record<string, unknown>)[key]);
            }
        }
    ...
    async install(dto: InstallComponentDto, client?: PoolClient): Promise<ComponentAssignment> {
        const executor = (client ?? this.db) as PgClient;
    ...
    async createCategory(dto: CreateComponentCategoryDto): Promise<ComponentCategory> {
        const result = await this.db.query(
            `INSERT INTO component_categories (code, name, description, parent_id, is_active, created_by)
            VALUES ($1, $2, $3, $4, $5, $6)
            RETURNING *`,
            [dto.code, dto.name, dto.description ?? null, dto.parentId ?? null, dto.isActive ?? true, dto.createdBy]
        );
        return mapRowToCategory(result.rows[0]);
    }

    async findCategoryById(id: string): Promise<ComponentCategory | null> {
        const result = await this.db.query('SELECT * FROM component_categories WHERE id = $1', [id]);
        if (result.rows.length === 0) return null;
        return mapRowToCategory(result.rows[0]);
    }

    async findCategoryByCode(code: string): Promise<ComponentCategory | null> {
        const result = await this.db.query('SELECT * FROM component_categories WHERE code = $1', [code]);
        if (result.rows.length === 0) return null;
        return mapRowToCategory(result.rows[0]);
    }
    ...
    async updateCategory(id: string, dto: UpdateComponentCategoryDto): Promise<ComponentCategory | null> {
        const fields: string[] = [];
        const params: unknown[] = [];
        let paramIndex = 1;

        if (dto.name !== undefined) { fields.push(`name = $${paramIndex++}`); params.push(dto.name); }
        if (dto.description !== undefined) { fields.push(`description = $${paramIndex++}`); params.push(dto.description); }
        if (dto.parentId !== undefined) { fields.push(`parent_id = $${paramIndex++}`); params.push(dto.parentId); }
        if (dto.isActive !== undefined) { fields.push(`is_active = $${paramIndex++}`); params.push(dto.isActive); }
    ...
    async createManufacturer(dto: CreateComponentManufacturerDto): Promise<ComponentManufacturer> {
        const result = await this.db.query(
            ...
            RETURNING *`,
            [
                dto.code, dto.name, dto.website ?? null, dto.supportUrl ?? null,
                dto.supportPhone ?? null, dto.supportEmail ?? null,
                dto.notes ?? null, dto.isActive ?? true, dto.createdBy
            ]
        );
    ...
    async findManufacturerById(id: string): Promise<ComponentManufacturer | null> {
        const result = await this.db.query('SELECT * FROM component_manufacturers WHERE id = $1', [id]);
        if (result.rows.length === 0) return null;
        return mapRowToManufacturer(result.rows[0]);
    }

    async findManufacturerByCode(code: string): Promise<ComponentManufacturer | null> {
        const result = await this.db.query('SELECT * FROM component_manufacturers WHERE code = $1', [code]);
        if (result.rows.length === 0) return null;
        return mapRowToManufacturer(result.rows[0]);
    }
    ...
    async updateManufacturer(id: string, dto: UpdateComponentManufacturerDto): Promise<ComponentManufacturer | null> {
        const fields: string[] = [];
        const params: unknown[] = [];
        let paramIndex = 1;

        if (dto.name !== undefined) { fields.push(`name = $${paramIndex++}`); params.push(dto.name); }
        if (dto.website !== undefined) { fields.push(`website = $${paramIndex++}`); params.push(dto.website); }
        if (dto.supportUrl !== undefined) { fields.push(`support_url = $${paramIndex++}`); params.push(dto.supportUrl); }
        if (dto.supportPhone !== undefined) { fields.push(`support_phone = $${paramIndex++}`); params.push(dto.supportPhone); }
        if (dto.supportEmail !== undefined) { fields.push(`support_email = $${paramIndex++}`); params.push(dto.supportEmail); }
        if (dto.notes !== undefined) { fields.push(`notes = $${paramIndex++}`); params.push(dto.notes); }
        if (dto.isActive !== undefined) { fields.push(`is_active = $${paramIndex++}`); params.push(dto.isActive); }

        if (fields.length === 0) return this.findManufacturerById(id);
