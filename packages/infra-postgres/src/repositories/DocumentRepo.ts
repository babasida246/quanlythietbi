/**
 * Knowledge Base Documents — Repository
 * Migrated from apps/api/src/modules/documents/documents.repository.ts
 */
...
export class DocumentRepo {
    ...
    async getById(id: string): Promise<KbDocument | null> {
    ...
    async update(id: string, patch: KbDocUpdateInput, actorUserId: string): Promise<KbDocument | null> {
    ...
    async submitApproval(id: string, actorUserId: string): Promise<KbDocument | null> {
        await this.db.query(
            `UPDATE documents
    ...
    async approve(id: string, actorUserId: string, reason: string | null): Promise<KbDocument | null> {
        await this.db.query(
            `UPDATE documents
    ...
    async reject(id: string, actorUserId: string, reason: string): Promise<KbDocument | null> {
        await this.db.query(
            `UPDATE documents
    ...
    async insertFileMeta(documentId: string, meta: KbDocFileMeta): Promise<KbDocument | null> {
        await this.db.query(
    ...
    private mapDocument(row: DocumentRow, files: DocumentFileRow[], relations: DocumentRelationRow[]): KbDocument {
        const scope = {
...
/** Minimal file row shape returned by getFile() */
export type { DocumentFileRow as KbDocFileRow }
