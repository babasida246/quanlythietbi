 ...
 * Migrated from apps/api/src/modules/labels/labels.service.ts
 */
...
export interface ILabelsRepository {
    createTemplate(dto: CreateTemplateDto, client?: unknown): Promise<LabelTemplate>;
    findTemplateById(id: string): Promise<LabelTemplate | null>;
    findTemplateByCode(code: string): Promise<LabelTemplate | null>;
    findDefaultTemplate(organizationId?: string): Promise<LabelTemplate | null>;
    updateTemplate(id: string, dto: UpdateTemplateDto): Promise<LabelTemplate | null>;
    hasActiveTemplates(): Promise<boolean>;
    findAllTemplates(query: TemplateListQuery & { isActive?: boolean }): Promise<{ data: LabelTemplateWithUsage[]; total: number }>;
    deleteTemplate(id: string): Promise<boolean>;
    setDefaultTemplate(id: string, organizationId?: string): Promise<boolean>;
    ...
    findPrintJobItems(jobId: string): Promise<PrintJobItemWithDetails[]>;
    updatePrintJobStatus(id: string, update: { status: string; errorMessage?: string; outputUrl?: string }): Promise<PrintJob | null>;
    getAssetLabelData(assetIds: string[]): Promise<Record<string, Record<string, unknown>>>;
    validateAssetFields(assetIds: string[], fields: string[]): Promise<FieldValidationResult[]>;
...
export class LabelsService {
    constructor(private repository: ILabelsRepository) { }
